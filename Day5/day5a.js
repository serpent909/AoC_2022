...
let rearrangeCrates = (cratesArray, instructionsArray) => {
    for (let i = 0; i < instructionsArray.length; i++) {
        ...
        let cratesRemaining = cratesArray[source].splice(numCratesToMove, cratesArray[source].length);
        let cratesRemoved = cratesArray[source].splice(0,numCratesToMove);

        cratesArray[source] = cratesRemaining;
        cratesArray[destination] = (cratesRemoved).concat((cratesArray[destination]))
    }
}
